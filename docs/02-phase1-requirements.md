...
#### 交互规则（保持宽高比时）
- **宽度 / 高度 / 百分比三个输入框自动联动**：用户输入其中任意一项，其余两项按基准图自动计算并回填：
  - 输入宽度 → 自动计算高度和百分比；
  - 输入高度 → 自动计算宽度和百分比；
  - 输入百分比 → 自动计算宽度和高度；
- 用户可实时看到缩放后的宽高（即联动回填的 width / height）。
- **缩放的最终依据是 width / height**：无论用户是直接输入宽高，还是通过百分比联动得到宽高，实际缩放都按 width / height 计算。百分比仅作为输入与显示方式，不直接参与缩放计算，因此不会因百分比取整而引入尺寸偏差。
- 不锁定宽高比时，必须同时填写宽度和高度，按用户指定尺寸缩放。

#### 软件需求
- 使用 Canvas 或 `pica` 库；
- 缩放以 width / height 为准，`percent` 仅在 UI 层用于联动输入，不传入尺寸计算；
- 如果只填宽度（或只填高度），按比例计算另一维；
- 如果宽高都填，按 fit 或 stretch 模式处理；
- 输出格式与原图一致或用户指定；
- 限制输出像素尺寸 ≤ 4096×4096。
